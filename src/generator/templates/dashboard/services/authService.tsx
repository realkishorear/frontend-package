...
export function useAuth(): AuthContextType {
  try {
    ...
    if (error?.message?.includes('AuthProvider context is undefined')) {
      console.warn('OIDC AuthProvider is not configured. Auth features will be disabled.')
      console.warn('To enable OIDC, configure your provider in .env file. See config/oidc.config.ts for instructions.')
      
      // Return a mock auth object that allows the app to continue working
      return {
        user: null,
        loading: false,
        isAuthenticated: false,
        signIn: async () => {
          console.warn('OIDC is not configured. Please set up your OIDC provider.')
        },
        signOut: async () => {
          console.warn('OIDC is not configured.')
        },
        signInRedirect: async () => {
          console.warn('OIDC is not configured. Please set up your OIDC provider.')
        },
